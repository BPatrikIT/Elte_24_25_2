module Week10 where
import Data.Char
import System.Win32.Console (COORD(xPos))
...
add2 :: Num a => [a] -> [a]
add2 xs = map' (\x -> x + 2) xs

-- Feladat: össtegezzük a rendezett párokat egy listába, amelynek a típusa [(a,b)].
addPairs :: Num a => [(a, a)] -> [a]
addPairs xs = map' (\(x,y) -> x + y) xs
...
indexedZipWith f = zipWith (\(i, x) y -> f i x y) . zip [0..]
